...
class FileHelper
{
    public static function partial($fileName, $fileTitle = null, $contentType = 'application/octet-stream')
    {
        ...
        if (isset($_SERVER['HTTP_RANGE']) && preg_match('%bytes=(\d+)-(\d+)?%i', $_SERVER['HTTP_RANGE'], $match)) {
            ...
            $crHeader = sprintf('Content-Range: bytes %d-%d/%d', $byteOffset, $finishBytes, $fileSize);

            header('HTTP/1.1 206 Partial content');
            header($crHeader);  //## Decrease by 1 on byte-length since this definition is zero-based index of bytes being sent
        }
