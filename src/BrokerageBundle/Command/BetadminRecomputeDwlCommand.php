...
class BetadminRecomputeDwlCommand extends ContainerAwareCommand
{
    public const COMMAND_NAME = 'brokerage:recompute:dwl';
    ...
    protected function executeDate(InputInterface $input, OutputInterface $output)
    {
        ...
        $dwlDate = \DateTimeImmutable::createFromFormat('Y-m-d', $input->getOption(self::OPTION_DWL_DATE));
                    ...
                    foreach ($members as $memberDetails) {
                        ...
                        try {
                            ...
                            if ($memberProduct) {
                                ...
                                $dwlSubtransactionIds[$dwl->getId()][] = $this->getRecomputeDwlService()->recomputeDWLForMemberProduct(
                                    $memberProduct,
                                    $dwl,
                                    $memberDetails['win_loss'],
                                    $memberDetails['stake'],
                                    $memberDetails['turnover'],
                                    $memberDetails['current_balance']
                                )->getId();
        ...
        $allSubtransactionlIds = $this->getSubTransactionRepository()->getDwlSubTransactioIds(array_keys($dwls));

        foreach ($allSubtransactionlIds as $subTransactionId) {
            if (!in_array($subTransactionId, $dwlSubtransactionIds)) {
                $this->getRecomputeDwlService()->changeToZeroSubtransaction($dwl, $subTransactionId);
            }
        }
    }
