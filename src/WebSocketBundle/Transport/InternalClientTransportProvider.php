...
namespace WebSocketBundle\Transport;
...
use Thruway\Transport\DummyTransportProvider;
...
class InternalClientTransportProvider extends AbstractRouterTransportProvider
{
    ...
    public function handleRouterStart(RouterStartEvent $event)
    {
        ...
        $clientTransport = new InternalClientTransport(function ($msg) use (&$session) {
            $session->dispatchMessage(Message::createMessageFromArray(json_decode(json_encode($msg), true)));
        }, $this->loop);

        $clientTransport->setSerializer(new \WebSocketBundle\Serializer\JsonSerializer());

        // create a new transport for the router side to use
        $transport = new InternalClientTransport(
            function ($msg) use ($clientTransport) {
                $this->internalClient->onMessage(
                    $clientTransport,
                    Message::createMessageFromArray(json_decode(json_encode($msg)), true)
                );
            },
            $this->loop
        );
